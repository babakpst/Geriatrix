...
// This data type packages all the required data to compute the DRM load at a
// point
...
public:
  ...
  struct InputLoad {
    ...
    int NNBndry; // Number of nodes on the DRM interface (always 1 in the 1D
                 // model)
    int NNLayer; // Number of nodes on the DRM layer (always 2 in the 1D model)
    int Wave_Type; // shear/pressure waves, in the 1D simulation, this does not
                   // matter
