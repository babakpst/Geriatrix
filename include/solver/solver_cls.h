...
class Solver_cls
    : public main_ns::Solver_ns::apply_seismic_loads_to_the_domain_cls {

  // members
private:
  std::ofstream info;    // information file
  std::ofstream FullSol; // file to write the entire solution at all times
  std::ofstream History; // file to write the history of the solution at some
                         // particular nodes
...
public:
  ...
  double Initial_Time; // Starting time of the simulation, usually negative,
                       // because of the DRM
...
private:
  ...
  virtual void Solve_the_system_for_this_RHS_using_Gaussina_Elimination(
      std::vector<double> &) = 0;
...
public:
  ...
  void
  solve_the_system_using_implicit_newmark_method(); // either using the skyline
                                                    // or full system
