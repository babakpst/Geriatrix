...
/*
###################################################################################################
Purpose: This function computes the load factor to apply the pressure load on
the surface.
...
================================= V E R S I O N
=================================================== V0.00: 06/26/2018 -
Subroutine initiated. V1.00: 06/26/2018 - Compiled successfully.

###################################################################################################
*/

double main_ns::Solver_ns::apply_seismic_loads_to_the_domain_cls::LoadFunction(
    const double Time, const double Alpha, const double P) {
...
================================= V E R S I O N
=================================================== V0.00: 06/26/2018 -
Subroutine initiated. V1.00: 06/27/2018 - Compiled successfully.

###################################################################################################
*/

void main_ns::Solver_ns::apply_seismic_loads_to_the_domain_cls::DRM_PointValues(
    main_ns::Solver_ns::apply_seismic_loads_to_the_domain_cls::InputLoad
        *LoadPackage) {
  ...
  case 0: // sine function
    ...
    wr = 2.0 * pi *
         LoadPackage->omega; // characteristic central circular frequency
  ...
  case 1: // Ricker
    ...
    // arg2 = wr * Time + wr * x/c ; // negative direction phase - reflected
    // wave
    ...
    if (LowerLimit <= arg1 && arg1 <= UpperLimit) {
      ...
      LoadPackage->u +=
          ...
          ((0.25 * ur * ur - 0.5) * exp(-0.25 * ur * ur) - 13.0 * exp(-13.5)) /
          (0.5 + 13.0 * exp(-13.5));

      LoadPackage->v +=
          LoadPackage->amplitude *
          (wr * (0.75 * ur - 0.125 * pow(ur, 3.0)) * exp(-0.25 * ur * ur)) /
          (0.5 + 13.0 * exp(-13.5));

      LoadPackage->a +=
          LoadPackage->amplitude *
          (pow(wr, 2.0) * (0.75 - 0.75 * pow(ur, 2.0) + 0.0625 * pow(ur, 4.0)) *
           exp(-0.25 * ur * ur)) /
          (0.5 + 13.0 * exp(-13.5));
    } else {
      for (int j = 2; j <= TotalCycle; j++) {
        ...
        if (LowerLimit <= arg1 && arg1 <= UpperLimit) {
          ...
          LoadPackage->u += (pow(Direction, (j - 1))) * LoadPackage->amplitude *
                            ((0.25 * ur * ur - 0.5) * exp(-0.25 * ur * ur) -
                             13.0 * exp(-13.5)) /
                            (0.5 + 13.0 * exp(-13.5));
          ...
          LoadPackage->a +=
              (pow(Direction, (j - 1))) * LoadPackage->amplitude *
              (pow(wr, 2.0) *
               (0.75 - 0.75 * pow(ur, 2.0) + 0.0625 * pow(ur, 4.0)) *
               exp(-0.25 * ur * ur)) /
              (0.5 + 13.0 * exp(-13.5));
        }
      }
...
================================= V E R S I O N
=================================================== V0.00: 07/04/2018 -
Subroutine initiated. V1.00: 07/04/2018 - Compiled successfully.
...
void main_ns::Solver_ns::apply_seismic_loads_to_the_domain_cls::
    DRM_Loads_Implicit(
        main_ns::Solver_ns::apply_seismic_loads_to_the_domain_cls::InputLoad
            *LoadPackage) {
  // the analytical acceleration at the boundary nodes. In this 1D problem,
  ...
  // Loop on the nodes on the DRM boundary to find out the analytical soln (In
  // 1D only one node)
  for (int i = 0; i < LoadPackage->NNBndry; i++) {
    for (int j = 0; j < LoadPackage->NDim; j++) {
      LoadPackage->x =
          LoadPackage
              ->XYZ[LoadPackage->NoBndry_DRM[i]][j]; // Coordinate of the node
      LoadPackage->u = LoadPackage->v = LoadPackage->a =
          0.0; // Initialize the values

      // Computing the analytical solution at this particular node
      // Remark: the one-dimensional wave-motion is identical for both SV and P
      // waves.
  ...
  for (int i = 0; i < LoadPackage->NNLayer * LoadPackage->NDim; i++) {
    for (int j = 0; j < LoadPackage->NNBndry * LoadPackage->NDim; j++) {
      F_e[i] += LoadPackage->M_eb[i][j] * Udd_b[j] +
                LoadPackage->C_eb[i][j] * Ud_b[j] +
                LoadPackage->K_eb[i][j] * U_b[j];
    }
  }
  ...
  // the analytical displacement at the boundary nodes. In this 1D problem,
  // there is only one node.
  std::vector<double> U_e(
      LoadPackage->NNLayer *
      LoadPackage
          ->NDim); // Not needed because we do not have damping in the system.
  ...
  // Loop on the nodes on the DRM layer to find out the analytical solution (In
  // 1D only two nodes)
  for (int i = 0; i < LoadPackage->NNLayer; i++) {
    for (int j = 0; j < LoadPackage->NDim; j++) {
      LoadPackage->x =
          LoadPackage
              ->XYZ[LoadPackage->NoLayer_DRM[i]][j]; // Coordinate of the node
      LoadPackage->u = LoadPackage->v = LoadPackage->a =
          0.0; // Initialize the values

      // Computing the analytical solution
      // Comment: the one-dimensional wave-motion is identical for both SV and P
      // waves.
      if (LoadPackage->Wave_Type == 0) // SV wave
        DRM_PointValues(LoadPackage);
      else if (LoadPackage->Wave_Type == 1) // P wave
  ...
  for (int i = 0; i < LoadPackage->NNBndry * LoadPackage->NDim; i++) {
    for (int j = 0; j < LoadPackage->NNLayer * LoadPackage->NDim; j++) {
      F_b[i] += -(LoadPackage->M_eb[j][i] * Udd_e[j] +
                  LoadPackage->C_eb[j][i] * Ud_e[j] +
                  LoadPackage->K_eb[j][i] * U_e[j]);
    }
  }
...
================================= V E R S I O N
=================================================== V0.00: 07/04/2018 -
Subroutine initiated. V1.00: 07/10/2018 - Compiled successfully.
...
void main_ns::Solver_ns::apply_seismic_loads_to_the_domain_cls::
  ...
  // The analytical solution is u (x,t) = u_i (exp(i k x) + exp (-i k x)) = 2
  // u_i cos(kx)
